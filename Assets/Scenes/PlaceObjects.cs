...
public class PlaceObjects : MonoBehaviour {
    ...
    public void Place() {
        ...
        numObjects = Random.Range(TerrainController.MinObjectsPerTile1, TerrainController.MaxObjectsPerTile1);
        for (int i = 0; i < numObjects; i++) {
            int prefabType = Random.Range(0, TerrainController.PlaceableObjects1.Length);
            Vector3 startPoint = RandomPointAboveTerrain();
            ...
            if (Physics.Raycast(startPoint, Vector3.down, out hit) && hit.point.y > TerrainController.Water.transform.position.y && hit.collider.CompareTag("Terrain")) {
                Quaternion orientation = Quaternion.Euler(Vector3.up * Random.Range(0f, 360f));
                RaycastHit boxHit;
                if (Physics.BoxCast(startPoint, TerrainController.PlaceableObjectSizes1[prefabType], Vector3.down, out boxHit, orientation) && boxHit.collider.CompareTag("Terrain")) {
                    Instantiate(TerrainController.PlaceableObjects1[prefabType], new Vector3(startPoint.x, hit.point.y, startPoint.z), orientation, transform);
                }
                //Debug code. To use, uncomment the giant thingy below
                //Debug.DrawRay(startPoint, Vector3.down * 10000, Color.blue);
                //DrawBoxCastBox(startPoint, TerrainController.PlaceableObjectSizes[prefabType], orientation, Vector3.down, 10000, Color.red);
